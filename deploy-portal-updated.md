...
| Task | Use This | Why |
|------|----------|-----|
| Deploy AI models (GPT-4o, embeddings) | **Azure AI Foundry** | Better model management UI |
| Create Azure resources (OpenAI, Storage, Search) | **Azure Portal** | Resource provisioning |
| Get API keys and endpoints | **Azure Portal** | Credentials management |
| Test models and chat | **Azure AI Foundry** | Built-in playground |
| Create Search indexes | **Azure Portal** | Search service management |
...
2. **Test in Playground (Optional)**
   - Click **"Chat"** in the left menu (or "Playground")
   - Select your `gpt-4o` deployment
   - Try a test message: "Hello, how are you?"
   - You should get a response confirming the deployment works
...
2. **Start Import and Vectorize Wizard**
   - Click "Import and vectorize data"

3. **Connect to Data Source**
   - **Data Source**: Select "Azure Blob Storage"
   - **Data source name**: `hr-policies-datasource`
   - **Connection string**: Paste the storage connection string from Step 8
   - **Container name**: `hr-policies`
   - Click "Next: Vectorize your text"

4. **Configure Vectorization**
   - **Vectorize text**: Toggle **ON** ✅
   - **Embedding model**: Select "Azure OpenAI"
   - **Subscription**: Your subscription
...
5. **Configure Advanced Settings**
   ...
   - **Parse mode**: `JSON` or `Default`
   - Click "Next: Create your index"

6. **Review Auto-Generated Index Schema**
   - **Index name**: Use auto-generated name or provide custom name
...
7. **Configure Semantic Ranking**
   - Enable **Semantic ranking** (toggle ON)
   - **Title field**: `title`
